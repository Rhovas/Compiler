...
data class MatchStmt(val name: String?, val exprs: List<Expr>, val cases: List<Pair<MatchPattern, Stmt>>): Stmt()
data class MatchPattern(val exprs: List<Expr>?, val isType: Type?)
data class ForStmt(val name: String, val expr: Expr, val stmt: Stmt): Stmt()
data class WhileStmt(val expr: Expr, val stmt: Stmt): Stmt()
data class TryStmt(val stmt: Stmt, val catch: Triple<String, Type, Stmt>?, val finally: Stmt?): Stmt()
